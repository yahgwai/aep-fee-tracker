...
// @ts-ignore - Will be used when refactoring outflow tests
function createOutflowData(
  address: string,
  overrides?: Partial<OutflowData>,
): OutflowData {
  return {
    metadata: {
      chain_id: CHAIN_IDS.ARBITRUM_ONE,
...
// @ts-ignore - Will be used when refactoring outflow tests
// Test data factory for outflow events
function createOutflowEvent(overrides?: Partial<OutflowEvent>): OutflowEvent {
  return {
...
// @ts-ignore - Will be used when refactoring outflow tests
// Test data factory for outflow entries
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function createOutflowEntry(
  overrides?: Partial<{
    block_number: number;
  ...
  const events = overrides?.events || [createOutflowEvent()];
  const totalWei = events
    .reduce((sum, event) => sum + BigInt(event.value_wei), BigInt(0))
    .toString();

  return {
    block_number: TEST_BLOCK_NUMBER,
    total_outflow_wei: overrides?.total_outflow_wei || totalWei,
    events,
  };
}
