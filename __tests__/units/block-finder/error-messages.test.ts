  ...
  describe("Descriptive error messages", () => {
    it("should provide full context when RPC connection fails", async () => {
      try {
        ...
        // The error can come from either initializeResult or getSafeCurrentBlock
        expect(blockError.message).toMatch(
          /Failed to get (network information from provider|current block number)/,
        );
        expect(blockError.context.cause).toBeInstanceOf(RPCError);
    ...
    it("should provide context when network info retrieval fails", async () => {
      try {
        ...
        // The error might come from getSafeCurrentBlock or initializeResult
        // depending on which executes first
        expect(error).toBeInstanceOf(BlockFinderError);
        const blockError = error as BlockFinderError;
        if (blockError.operation === "initializeResult") {
  ...
  describe("RPCError context", () => {
    it("should include retry count and operation in RPCError", async () => {
      try {
        ...
        expect(rpcError.retryCount).toBe(3);
        expect(rpcError.operation).toBe("get current block number");
